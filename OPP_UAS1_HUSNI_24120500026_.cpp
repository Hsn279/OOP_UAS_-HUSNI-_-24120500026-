...
public:
    Person(string name,
           ...
           string idCard,
           string taxId)
        : name(name), address(address), dateOfBirth(DOB), gender(gender), NoPhone(phone), Email(email), Nationality(Nationality), IdCard(idCard), taxId(taxId) {}
...
public:
    Coach(string name,
        ...
        : Person(name, address, DOB, gender, phone, email, nationality, IdCard, TaxId),
          license(license), role(role), Experience(experience) {}
    ...
    void displayCoachInfo() const {
        displayInfo();
        cout << "Role: " << role << ", License: " << license << endl;
    }
};
...
public:
    Player(string name,
           ...
           int assists)
        : Person(name, address, DOB, gender, phone, email, nationality, IdCard, TaxId),
          NumberShirt(number), position(pos), goalsScored(goals), assistsMade(assists) {}

    void setAssignedTeam(shared_ptr<Team> team) {
        assignedTeam = team;
    }

    string getNumberShirt() const { return NumberShirt; }

    void displayPlayerInfo() const {
        displayInfo();
        cout << "Number: " << NumberShirt << ", Position: " << position << endl;
    }
};
...
public:
    Staff(string Name,
          ...
          string Dept)
        : Person(Name, address, DOB, gender, phone, email, nationality, IdCard, TaxId),
          StaffPosition(Staff), Department(Dept) {}

    string getStaffPosition() const { return StaffPosition; }

    void displayStaffInfo() const {
        displayInfo();
        cout << "Staff : " << StaffPosition << ", Department: " << Department << endl;
    }
};
...
public:
    ...
    void displayTeamInfo() const {
        cout << "Team Name: " << Clubname << ", League: " << league << endl;
        if (headCoach) {
        ...
        for (const auto& player : players) {
            if (player) cout << "    - " << player->getName() << " (" << player->getNumberShirt() << ")" << endl;
        }
        cout << "  Staff Members (" << staffMembers.size() << "):" << endl;
        for (const auto& staff : staffMembers) {
            if (staff) cout << "    - " << staff->getName() << " (" << staff->getStaffPosition() << ")" << endl;
        }
    }
};
...
int main() {
    ...
    shared_ptr<Coach> headCoach = make_shared<Coach>(
    ...
    shared_ptr<Coach> assistantCoach = make_shared<Coach>(
    ...
    shared_ptr<Staff> physiotherapist = make_shared<Staff>(
    ...
    shared_ptr<Match> match1 = make_shared<Match>("MD01", "2025-07-07", "15:00", "ITL TRISAKTI", "Win");
    match1->setTeams(fcCakrawalaMuda, make_shared<Team>("ITL TRISAKTI", "Liga Mahasiswa", "U-23", 2020, "Stadion Cakrawala", "Merah-Hitam"));
